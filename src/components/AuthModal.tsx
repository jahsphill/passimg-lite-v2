...
import { ShieldCheck, UserCircle, Edit3, Mail, Lock, Sparkles, Plus, Wallet, Save, Upload } from 'lucide-react';
...
export default function AuthModal({
  ...
  const handleProfileFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 2 * 1024 * 1024) {
        showFeedback("Profile picture size must be under 2MB", "error");
        return;
      }
      const reader = new FileReader();
      reader.onloadend = () => {
        setProfileAvatar(reader.result as string);
        showFeedback("Profile picture uploaded!", "success");
      };
  ...
  const handleEmailSignUp = (e: React.FormEvent) => {
    ...
    console.log("[SIGNUP EVENT] User signup flow started with name (Modal):", signupName, "email:", signupEmail);
    ...
    console.log("[PROFILE EVENT] Created local Sandbox profile inside AuthModal with onboardingCompleted = false for User:", newUser.id);
    onSignUp(newUser);
    console.log("[SESSION EVENT] Active sandbox session established for modal user:", newUser.id);
    onLogin(newUser);
    showFeedback('Registration successful! Welcome.', 'success');
    console.log("[REDIRECT EVENT] Closing Modal. Routing user based on global router effect.");
    onClose();
  };
              ...
              <div className="space-y-3 pb-3 border-b border-white/10">
                <div className="flex items-center space-x-4">
                  <img 
                    ...
                    alt="Avatar preview" 
                    className="h-16 w-16 rounded-xl object-cover border border-white/15 shadow-md shrink-0"
                  ...
                  <div 
                    ...
                    className={`flex-1 border border-dashed rounded-xl p-3 text-center transition-all ${
                      profileDragging 
                  ...
                  >
                    ...
                    >
                      ...
                      <p className="text-[10px] text-white font-medium">
                        Click / Drag-and-drop to custom upload
                      </p>
                    </label>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-[10px] font-bold tracking-wider uppercase text-slate-400">Or Image URL</label>
                  <input
                    type="text"
